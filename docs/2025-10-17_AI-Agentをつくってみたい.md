...
| ツール | 機能 | 状態 |
|--------|------|------|
| `SearchMessages` | 新Guild Search API使用、サーバー全体を高速検索 | ✅ |
| `GetChannelInfo` | チャンネル情報取得 | ✅ |
| `GetThreadContext` | スレッドの会話履歴取得 | ✅ |
| `GetMessagesAround` | 特定メッセージの前後取得 | ✅ |
...
2. プログレス表示改善（Discordリアクション）
3. より複雑な質問でのテストとプロンプト改善
